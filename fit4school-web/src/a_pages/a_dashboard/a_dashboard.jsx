    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 p-3 sm:p-4 md:p-6 lg:p-8 overflow-x-hidden">
          ...
          {/* Announcements & Queue/Activity sections (left as UI; bound to realtime data below) */}
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="col-span-1 lg:col-span-1 bg-white rounded-lg shadow p-4">
              ...
              <div className="mb-3">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={handlePostAnnouncement} className="px-3 py-1 bg-cyan-500 text-white rounded">Post</button>
                </div>
              </div>
                  ...
                  <div key={a.id} className="border rounded p-2">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">{typeof a.date === "string" ? a.date : formatDateTime(a.date)} • {a.author || "Admin"}</p>
                      </div>
                      <button onClick={() => handleDeleteAnnouncement(a.id)} className="text-xs text-red-500">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="col-span-1 lg:col-span-1 bg-white rounded-lg shadow p-4">
              <h4 className="font-bold mb-2">Now Serving</h4>
              {currentCustomer ? (
                <div>
                  <p className="text-lg font-semibold">{currentCustomer.name}</p>
                  <p className="text-sm text-gray-600">Queue: {currentCustomer.queueNo}</p>
                  <p className="text-sm text-gray-600">Orders: {currentCustomer.totalOrders}</p>
                  <div className="mt-2 space-y-1">
                    {(currentCustomer.orders || []).map((it, idx) => (
                      <div key={idx} className="text-sm text-gray-700 border rounded p-2">
                        <div className="flex justify-between">
                          <div>{it.item} ({it.size})</div>
                ...
                <p className="text-sm text-gray-500">No one is being served right now</p>
              )}

              <hr className="my-3" />

              <h4 className="font-bold mb-2">Queue</h4>
              <div className="space-y-2 max-h-48 overflow-auto">
                {queueList.length === 0 && <p className="text-sm text-gray-500">Queue empty</p>}
                {queueList.map((q) => (
                  <div key={q.id} className="flex justify-between items-center border rounded p-2">
                    <div>
                      <div className="text-sm font-semibold">{q.name}</div>
                      <div className="text-xs text-gray-500">Orders: {q.orders} • Wait: {q.waitTime}</div>
                    </div>
                    <div className="text-sm text-gray-700 font-mono">#{q.queueNo}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {recentActivity.map((r) => (
                  <div key={r.id} className="flex justify-between items-center border rounded p-2">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{r.action} • {r.time}</div>
                    </div>
                    <div>
                      <span className={`inline-block px-2 py-1 text-xs rounded ${r.status === "success" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                        {r.status === "success" ? "Completed" : "Missed"}
                      </span>
                    </div>
                  </div>
                ))}
