          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                <div className="relative w-40 h-40 md:w-48 md:h-48">
                  <svg viewBox="0 0 100 100" className="transform -rotate-90">
                    {(() => {
                      ...
                      const k = gradeDistribution.Kindergarten;
                      const e = gradeDistribution.Elementary;
                      const j = gradeDistribution["Junior High"];
                      
                      // Calculate percentages
                      const kPercent = (k / total) * 100;
                      const ePercent = (e / total) * 100;
                      const jPercent = (j / total) * 100;
                      
                      const circumference = 2 * Math.PI * 40;

                      // Calculate dash arrays
                      const kDash = (kPercent / 100) * circumference;
                      const eDash = (ePercent / 100) * circumference;
                      const jDash = (jPercent / 100) * circumference;
                        ...
                        <>
                          ...
                          {k > 0 && (
                            <circle 
                              ...
                              stroke="#00C951" 
                              strokeWidth="20" 
                              strokeDasharray={`${kDash} ${circumference - kDash}`}
                              strokeDashoffset="0" 
                            />
                          )}
                          {/* Elementary - Blue */}
                          ...
                          {j > 0 && (
                            <circle 
                              ...
                              stroke="#ef4444" 
                              strokeWidth="20" 
                              strokeDasharray={`${jDash} ${circumference - jDash}`}
                              strokeDashoffset={`-${kDash + eDash}`} 
                            />
                          )}
                        </>
                      );
            ...
            <div className="bg-white rounded-lg shadow-md p-4 md:p-6 transition">
              ...
              <div className="flex flex-col md:flex-row items-center justify-around">
                ...
                <div className="relative w-40 h-40 md:w-48 md:h-48">
                  <svg viewBox="0 0 100 100" className="transform -rotate-90">
                    {(() => {
                      ...
                      const categories = Object.keys(uniformDistribution);
                      const values = Object.values(uniformDistribution);
                      
                      const circumference = 2 * Math.PI * 40;
                      const colors = ['#64f56e', '#36A2EB', '#61ab66', '#4BC0C0', '#4cf527', '#2f6f99', '#00af6b', '#79f1f6'];
                ...
                {/* Legend */}
                <div className="flex flex-col gap-3 mt-4 md:mt-0 max-h-40 overflow-y-auto">
                  {Object.entries(uniformDistribution).map(([category, count], index) => {
                    const colors = ['#64f56e', '#36A2EB', '#61ab66', '#4BC0C0', '#4cf527', '#2f6f99', '#00af6b', '#79f1f6'];
                    ...
                    return (
                      <div key={category} className="flex items-center gap-2">
                        ...
                        ></div>
                        <span className="text-sm text-gray-700">
                          {category.replace('_', ' ')} - {count} ({uniformTotal > 0 ? Math.round((count/uniformTotal)*100) : 0}%)
                        </span>
                      </div>
                    );
                  })}
